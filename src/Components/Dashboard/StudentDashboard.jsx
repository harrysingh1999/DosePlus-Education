...
export default function StudentDashboard() {
    ...
    const [marksValues, setMarksValues] = useState({ math: "", english: "", hindi: "", science: "", sst: "", total: "", per: "", remarks: '' })
  const [marksValues2, setMarksValues2] = useState({ math: "", english: "", hindi: "", science: "", sst: "" })
  const [keyVaue, setKeyVaue] = useState([])
  ...
  const { name, rollNo, userType, schoolName, section, fatherName, myClass, dob, id } = userData;
...
console.log(marksValues);
  const getResult = async () => {
    try {
      let response = await fetch(`http://localhost:8080/api/rollno/${rollNo}`);
      let jsData = await response.json();
      setMarksValues({ math: jsData?.mathsScore, english: jsData?.englishScore, hindi: jsData?.hindiScore, science: jsData?.scienceScore, sst: jsData?.socialScienceScore, total: jsData?.totalMarks, per: jsData?.totalPercentage, remarks: jsData?.remarks  })
  ...
  const handleMarks = (e) => {
    ...
    setKeyVaue([...keyVaue, (+value)])
    if (!checked) {
      const filterValue = keyVaue.filter((item) => item !== (+value))
      setKeyVaue(filterValue)
  ...
  const totalHandler = (e) => {
    ...
    if (checked) {
      setKeyVaue([])
      setMarksValues2({ math: "", english: "", hindi: "", science: "", sst: "" })
    }
    if (!checked) {
      setTotal({ total: 0 })
  ...
  useEffect(() => {
    const sum = keyVaue.reduce((acc, curr) => acc + curr, 0) / (100 * keyVaue.length) * 100 || 0
  ...
  useEffect(() => {
    ...
    return (
    <>
      { marksValues.math ? <div className="grid grid-cols-12 px-12 py-[120px] ">
        <div className="col-span-9 relative">
          <div className="w-full h-full absolute top-0 flex justify-center items-center">
            <div className="bg-white rounded-lg p-10">
              <div className="flex flex-row gap-60  px-28">
                <div className="flex flex-col justify-between items-center">
                  <h1 className="text-[#a80946] text-3xl font-bold mb-6">Score Card</h1>
                  <div className="h-28 w-28 rounded-full border-[10px] mb-10 border-[#fd0000] grid place-items-center">
                    <span className="font-bold text-2xl text-[#fd0000]">{total?.total ? total?.total : values}%</span>
                  </div>
                </div>

                <div className="flex flex-col gap-4">
                  <div>
                    <h3 className="text-xl font-semibold">Marks by Subject</h3>
                  </div>
                  <ul>
                    <li className="flex items-center" >
                      <input
                        ...
                        checked={marksValues2.math || marksValues2.math === 0 ? true : false}
                      />
                    ...
                    <li className="flex items-center" >
                      <input
                        ...
                        checked={marksValues2.english || marksValues2.english === 0 ? true : false}
                    ...
                    <li className="flex items-center" >
                      <input
                        ...
                        checked={marksValues2.hindi || marksValues2.hindi === 0 ? true : false}
                    ...
                    <li className="flex items-center" >
                      <input
                        ...
                        onChange={handleMarks}
                        checked={marksValues2.science || marksValues2.science === 0 ? true : false}
                      />
                    ...
                    <li className="flex items-center" >
                      <input
                        ...
                        onChange={handleMarks}
                        checked={marksValues2.sst || marksValues2.sst === 0 ? true : false}
                        className="checked:bg-blue-600 checked:border-transparent"
                      />
                      <span className="ml-2">Social Science</span>
                    </li>
                    <li className="flex items-center">
                      <input
                        type="checkbox"
              ...
              <div className="">
                <div className="flex flex-row items-center">
                  ...
                  <p className={`w-full font-semibold h-[35px] border-b-2 border-gray-400 px-2 flex flex-row items-center ${marksValues.remarks === 'Fail' ? 'text-red-500' : 'text-green-400'}`}>
        ...
        <div className="col-span-3">
          <div className="py-3 px-4 flex flex-col gap-7">
            <h2 className="text-[#b40047] text-[24px] font-semibold">Profile Details</h2>
            ...
            <div className="border-2 border-slate-500 rounded-md py-2 px-4 w-full">
              ...
              <span className="text-[16px] text-slate-500 ps-2">{fatherName}</span>
            </div>
            <div className="border-2 border-slate-500 rounded-md py-2 px-4 w-full">
            ...
            <div className="border-2 border-slate-500 rounded-md py-2 px-4 w-full">
              ...
              <span className="text-[16px] text-slate-500 ps-2">{schoolName}</span>
            </div>
        ...
        <div className="h-screen text-center flex items-center justify-center">
          <p className="text-red-400 text-2xl">Oops Student Score not Found. Please ask the admin to add your Score.</p>
        </div>
      ) }
    </>
  )
